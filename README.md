...
Los envíos reales no usan teléfonos inventados. Configura `UTEL_TEST_PHONES_JSON` en el archivo `.env` con un banco de números activos y expresamente autorizados por país. Se aceptan valores nacionales o E.164; el backend los valida contra el plan telefónico del país, elimina el código internacional porque el formulario ya lo agrega y reserva un número distinto para cada fila. Si falta un país, un número pertenece a otra región o el banco no alcanza para el lote, el proceso se detiene antes del primer envío. Los números generados automáticamente se conservan únicamente para `dry run`.
