...
export function buildErrorLog(job) {
  ...
  failures.forEach(({ item, result, stage }, index) => {
    ...
    lines.push(
      ...
      `Nivel solicitado: ${result.level || row.level || "No disponible"}`,
      `Modalidad solicitada: ${result.modality || row.modality || "En linea (por defecto)"}`,
      `Modalidad real observada: ${result.selected_modality || result.navigation_modality || "No registrada"}`,
      `Programa seleccionado: ${result.selected_program_name || "No seleccionado"}`,
      `Programa esperado: ${row.program_name || "No disponible"}`,
      `Modo: ${result.dry_run ? "DRY RUN - SIN ENVÍO" : "ENVÍO REAL"}`,
      `POST UTEL realizado: ${result.utel_submission_attempted === true ? "Sí" : result.utel_submission_attempted === false ? "No" : "Desconocido"}`,
      `Estado del envío: ${result.utel_submission || "No disponible"}`,
      `Aviso del envío: ${result.utel_submission_message || "No disponible"}`,
      `URL de entrada: ${row.utel_url || "No disponible"}`,
      `URL final observada: ${result.utel_url || stage.url || "No disponible"}`,
